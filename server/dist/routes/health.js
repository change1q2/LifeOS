...
router.delete('/api/health/habits/:id', authMiddleware, (req, res) => {
    ...
    db.tables.health_habits = (db.tables.health_habits || []).filter((h) => {
        const idMatch = Number(h.id) === targetId;
        const userMatch = h.user_id === req.userId;
        return !(idMatch && userMatch);
    });
    db.tables.habit_records = (db.tables.habit_records || []).filter((r) => {
...
router.delete('/api/health/logs/:id', authMiddleware, (req, res) => {
    const db = getDb();
    const targetId = Number(req.params.id);
    db.tables.health_logs = (db.tables.health_logs || []).filter((item) => {
